...
Всё локально, **офлайн-first**: без интернета работает всё, кроме подтягивания метаданных из сети. Обложки и найденные метаданные складываются на диск, поэтому один раз занесённая полка выглядит и ищется одинаково с сетью и без.
...
- ✍️ **Добавление вручную** — с онлайн-поиском по названию или ISBN и автозаполнением полей, либо целиком руками.
- 📥 **Импорт из Goodreads и LiveLib** — CSV-выгрузка заезжает целиком, с авторами, оценками и датами прочтения; дубли по ISBN пропускаются.
...
- 🤝 **Выдача** — «выдана: Маша, ждём до 2026-09-01»; просрочка подсвечивается, поиск помечает, что книги нет на месте.
- 📅 **Даты чтения** — начал и дочитал; статистика показывает, сколько прочитано в каждом году.
- 📊 **Статистика** — всего книг, страниц прочитано, книг на руках, просрочено, по статусам, по годам и топ-жанры.
- 🖼 **Обложки на диске** — скачиваются один раз, дальше полка выглядит нормально и без интернета.
- ♻️ **Кэш метаданных** — каждый ISBN спрашивается у источников один раз за всё время.
- 📤 **Экспорт в CSV** — весь каталог одним файлом, с путём до полки и BOM (Excel не ломает кириллицу).
- 💾 **Резервная копия** — экспорт и восстановление каталога в JSON.
...
Опрашиваются по очереди, до первого непустого ответа. Первые три работают **без ключей и регистрации**:

| Источник | Ключ | Сильная сторона |
|---|:---:|---|
| [Open Library](https://openlibrary.org) | — | англоязычные книги; русские часто в романизации |
| [Deutsche Nationalbibliothek](https://www.dnb.de) (SRU) | — | немецкие и европейские издания |
| [Library of Congress](https://www.loc.gov) (SRU) | — | крупнейший англоязычный каталог |
| [Google Books](https://developers.google.com/books) | опц. | лучшее покрытие русских книг |

Google стоит последним намеренно: **без ключа он почти всегда отвечает 429** — общая анонимная квота выжрана круглосуточно. Ключ бесплатный и заводится за пять минут (см. раздел «Настройки» в приложении).

Все провайдеры скрыты за трейтом `MetadataProvider` и мокаются в тестах — сеть в юнит-тестах запрещена, парсинг проверяется на сохранённых JSON- и XML-фикстурах.
