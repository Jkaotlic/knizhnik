...
- **macOS** — `knizhnik_x.y.z_universal.dmg` (Intel + Apple Silicon).

### macOS: приложение «повреждено» — что делать

Приложение пока **не подписано у Apple**, поэтому Gatekeeper вешает на скачанный `.dmg` метку карантина и при первом запуске говорит, что программа «повреждена и не может быть открыта». Файл в порядке — так macOS отвечает на любую сборку без подписи Developer ID.

1. Открой `.dmg` и перетащи **knizhnik** в папку «Программы».
2. Сними карантин — в Терминале:

   ```bash
   xattr -dr com.apple.quarantine /Applications/knizhnik.app
   ```

3. Запусти приложение обычным способом.

Флаг `-r` обязателен: `.app` — это папка, и метка висит не только на ней, но и на вложенных файлах. Команду нужно выполнять **после** перетаскивания в «Программы», а не на файле внутри примонтированного образа — иначе метка приедет вместе с копией.

**Без терминала:** правый клик по приложению → **Открыть** → «Всё равно открыть». Достаточно одного раза. Либо `System Settings → Privacy & Security → Open Anyway`.

Проверить, стоит ли ещё карантин: `xattr -l /Applications/knizhnik.app`.
